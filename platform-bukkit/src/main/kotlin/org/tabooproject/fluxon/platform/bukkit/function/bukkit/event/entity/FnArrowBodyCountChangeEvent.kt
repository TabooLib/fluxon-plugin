    ...
    private fun init() {
        with(FluxonRuntime.getInstance()) {
            registerExtension(ArrowBodyCountChangeEvent::class.java)
                .function("isReset", returns(Type.Z).noParams()) { it.setReturnRef(it.target?.isReset) }
                .function("oldAmount", returnsObject().noParams()) { it.setReturnRef(it.target?.oldAmount) }
                .function("newAmount", returnsObject().noParams()) { it.setReturnRef(it.target?.newAmount) }
                .function("setNewAmount", returnsObject().params(Type.OBJECT)) { it.setReturnRef(it.target?.setNewAmount(it.getInt(0).toInt())) }
