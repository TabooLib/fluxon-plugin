    ...
    private fun init() {
        with(FluxonRuntime.getInstance()) {
            registerExtension(FireworkMeta::class.java)
                .function("addEffect", returnsObject().params(Type.OBJECT)) { it.setReturnRef(it.target?.addEffect(it.getRef(0) as FireworkEffect)) }
                ...
                .function("effectsSize", returnsObject().noParams()) { it.setReturnRef(it.target?.effectsSize) }
                .function("removeEffect", returnsObject().params(Type.OBJECT)) { it.setReturnRef(it.target?.removeEffect(it.getInt(0).toInt())) }
                .function("clearEffects", returnsObject().noParams()) { it.setReturnRef(it.target?.clearEffects()) }
                .function("hasEffects", returns(Type.Z).noParams()) { it.setReturnRef(it.target?.hasEffects()) }
