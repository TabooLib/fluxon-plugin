    ...
    private fun init() {
        with(FluxonRuntime.getInstance()) {
            registerExtension(InventoryView::class.java)
                ...
                .function("setItem",returnsVoid().params(Type.I, org.tabooproject.fluxon.platform.bukkit.function.bukkit.inventory.FnItemStack.TYPE)) { it.target?.setItem(it.getAsInt(0).toInt(), it.getRef(1) as ItemStack) }
                ...
                .function("getInventory",returns(org.tabooproject.fluxon.platform.bukkit.function.bukkit.inventory.FnInventory.TYPE).params(Type.I)) { it.setReturnRef(it.target?.getInventory(it.getAsInt(0).toInt())) }
                .function("convertSlot", returns(Type.I).params(Type.I)) { it.setReturnInt(it.target?.convertSlot(it.getAsInt(0).toInt()) ?: 0) }
...
@PlatformSide(Platform.BUKKIT)
object FnInventoryViewProperty : org.tabooproject.fluxon.platform.bukkit.function.FnEnumGetter<org.bukkit.inventory.InventoryView.Property>(org.bukkit.inventory.InventoryView.Property::class.java) {
