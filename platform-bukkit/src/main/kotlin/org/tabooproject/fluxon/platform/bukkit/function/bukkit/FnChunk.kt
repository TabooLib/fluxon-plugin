    ...
    private fun init() {
        with(FluxonRuntime.getInstance()) {
            registerExtension(Chunk::class.java)
                ...
                .function("isEntitiesLoaded", returns(Type.Z).noParams()) { it.setReturnRef(it.target?.isEntitiesLoaded) }
                ...
                .function("isGenerated", returns(Type.Z).noParams()) { it.setReturnRef(it.target?.isGenerated) }
                .function("isLoaded", returns(Type.Z).noParams()) { it.setReturnRef(it.target?.isLoaded) }
                ...
                .function("isSlimeChunk", returns(Type.Z).noParams()) { it.setReturnRef(it.target?.isSlimeChunk) }
                .function("isForceLoaded", returns(Type.Z).noParams()) { it.setReturnRef(it.target?.isForceLoaded) }
                .function("setForceLoaded", returnsObject().params(Type.OBJECT)) { it.setReturnRef(it.target?.setForceLoaded(it.getBool(0))) }
                .function("addPluginChunkTicket", returnsObject().params(Type.OBJECT)) { it.setReturnRef(it.target?.addPluginChunkTicket(it.getRef(0) as Plugin)) }
                .function("removePluginChunkTicket", returnsObject().params(Type.OBJECT)) { it.setReturnRef(it.target?.removePluginChunkTicket(it.getRef(0) as Plugin)) }
                .function("pluginChunkTickets", returnsObject().noParams()) { it.setReturnRef(it.target?.pluginChunkTickets) }
                .function("inhabitedTime", returnsObject().noParams()) { it.setReturnRef(it.target?.inhabitedTime) }
                .function("setInhabitedTime", returnsObject().params(Type.OBJECT)) { it.setReturnRef(it.target?.setInhabitedTime(it.getInt(0).toLong())) }
