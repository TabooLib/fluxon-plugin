    ...
    private fun init() {
        with(FluxonRuntime.getInstance()) {
            registerExtension(Inventory::class.java)
                ...
                .function("setMaxStackSize", returnsVoid().params(Type.I)) { it.target?.setMaxStackSize(it.getAsInt(0).toInt()) }
                .function("getItem", returns(org.tabooproject.fluxon.platform.bukkit.function.bukkit.inventory.FnItemStack.TYPE).params(Type.I)) { it.setReturnRef(it.target?.getItem(it.getAsInt(0).toInt())) }
                .function("setItem",returnsVoid().params(Type.I, org.tabooproject.fluxon.platform.bukkit.function.bukkit.inventory.FnItemStack.TYPE)) {
                    it.target?.setItem(it.getAsInt(0).toInt(), it.getRef(1) as ItemStack)
                }
                ...
                .function("contains", returns(Type.Z).params(org.tabooproject.fluxon.platform.bukkit.function.bukkit.FnMaterial.TYPE, Type.I)) { it.setReturnBool(it.target?.contains(it.getRef(0) as Material, it.getAsInt(1).toInt()) ?: false) }
                .function("contains", returns(Type.Z).params(org.tabooproject.fluxon.platform.bukkit.function.bukkit.inventory.FnItemStack.TYPE, Type.I)) { it.setReturnBool(it.target?.contains(it.getRef(0) as ItemStack, it.getAsInt(1).toInt()) ?: false) }
                ...
                .function("clear", returnsVoid().params(Type.I)) { it.target?.clear(it.getAsInt(0).toInt()) }
                ...
                .function("iterator", returns(Type.fromClass(java.util.ListIterator::class.java)).params(Type.I)) { it.setReturnRef(it.target?.iterator(it.getAsInt(0).toInt())) }
