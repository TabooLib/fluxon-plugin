    ...
    private fun init() {
        with(FluxonRuntime.getInstance()) {
            registerExtension(EntityTeleportEvent::class.java)
                ...
                .function("setFrom", returnsObject().params(Type.OBJECT)) { it.setReturnRef(it.target?.setFrom(it.getRef(0) as Location)) }
                .function("to", returnsObject().noParams()) { it.setReturnRef(it.target?.to) }
                .function("setTo", returnsObject().params(Type.OBJECT)) { it.setReturnRef(it.target?.setTo(it.getRef(0) as Location)) }
