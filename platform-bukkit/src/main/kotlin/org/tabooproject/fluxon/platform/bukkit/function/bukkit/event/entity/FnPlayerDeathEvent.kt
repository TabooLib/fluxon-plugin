    ...
    private fun init() {
        with(FluxonRuntime.getInstance()) {
            registerExtension(PlayerDeathEvent::class.java)
                ...
                .function("setNewExp", returnsVoid().params(Type.I)) { it.target?.setNewExp(it.getAsInt(0).toInt()) }
                .function("newLevel", returns(Type.I).noParams()) { it.setReturnInt(it.target?.newLevel ?: 0) }
                .function("setNewLevel", returnsVoid().params(Type.I)) { it.target?.setNewLevel(it.getAsInt(0).toInt()) }
                .function("newTotalExp", returns(Type.I).noParams()) { it.setReturnInt(it.target?.newTotalExp ?: 0) }
                .function("setNewTotalExp", returnsVoid().params(Type.I)) { it.target?.setNewTotalExp(it.getAsInt(0).toInt()) }
