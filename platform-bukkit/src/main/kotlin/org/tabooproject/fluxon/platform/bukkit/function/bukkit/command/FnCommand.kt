...
//                .function("setTimings", returnsObject().params(Type.OBJECT)) { it.setReturnRef(it.target?.timings = it.getRef(0) as CustomTimingsHandler) }
                .function("execute", returns(Type.Z).params(Type.OBJECT, Type.STRING, Type.OBJECT)) {
                ...
                .function("tabComplete", returnsObject().params(Type.OBJECT, Type.STRING, Type.OBJECT, Type.OBJECT)) {
                    it.setReturnRef(it.target?.tabComplete(
                        it.getRef(0) as CommandSender,
                        it.getString(1)!!,
                        it.getRef(2) as Array<String>,
                ...
                .function("testPermission", returns(Type.Z).params(Type.OBJECT)) {
                    it.setReturnBool(it.target?.testPermission(it.getRef(0) as CommandSender) ?: false)
                }
                .function("testPermissionSilent", returns(Type.Z).params(Type.OBJECT)) {
                ...
                .function("unregister", returns(Type.Z).params(Type.OBJECT)) {
                ...
                .function("broadcastCommandMessage", returnsVoid().params(Type.OBJECT, Type.STRING)) {
                    Command.broadcastCommandMessage(
                        it.getRef(0) as CommandSender,
                        it.getString(1)!!
                    )
                }
                .function("broadcastCommandMessage", returnsVoid().params(Type.OBJECT, Type.STRING, Type.Z)) {
                    Command.broadcastCommandMessage(
                        it.getRef(0) as CommandSender,
                        it.getString(1)!!,
                        it.getBool(2)
