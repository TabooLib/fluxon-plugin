...
@Requires(classes = ["org.bukkit.conversations.InactivityConversationCanceller"])
@PlatformSide(Platform.BUKKIT)
object FnInactivityConversationCanceller {

    val TYPE = Type.fromClass(org.bukkit.conversations.InactivityConversationCanceller::class.java)

    @Awake(LifeCycle.INIT)
    private fun init() {
        with(FluxonRuntime.getInstance()) {
            registerExtension(org.bukkit.conversations.InactivityConversationCanceller::class.java)
                ...
                // .function("startTimer", returnsVoid().noParams()) { it.target?.startTimer() }
                // .function("stopTimer", returnsVoid().noParams()) { it.target?.stopTimer() }
                // .function("cancelling", returnsVoid().params(org.tabooproject.fluxon.platform.bukkit.function.bukkit.conversations.FnConversation.TYPE)) { it.target?.cancelling(it.getRef(0) as org.bukkit.conversations.Conversation) }
        }
    }
}
