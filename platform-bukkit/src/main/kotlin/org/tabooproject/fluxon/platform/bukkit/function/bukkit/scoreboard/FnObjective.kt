    ...
    private fun init() {
        with(FluxonRuntime.getInstance()) {
            registerExtension(Objective::class.java)
                ...
                .function("trackedCriteria", returnsObject().noParams()) { it.target?.trackedCriteria }
                .function("isModifiable", returns(Type.Z).noParams()) { it.target?.isModifiable }
                ...
                .function("setDisplaySlot", returnsObject().params(Type.OBJECT)) { it.target?.setDisplaySlot(it.getRef(0) as DisplaySlot) }
                .function("displaySlot", returnsObject().noParams()) { it.target?.displaySlot }
                .function("setRenderType", returnsObject().params(Type.OBJECT)) { it.target?.setRenderType(it.getRef(0) as RenderType) }
                .function("renderType", returnsObject().noParams()) { it.target?.renderType }
