    ...
    private fun init() {
        with(FluxonRuntime.getInstance()) {
            registerExtension(Conduit::class.java)
                ...
                .function("frameBlocks", returns(StandardTypes.COLLECTION).noParams()) { it.setReturnRef(it.target?.frameBlocks) }
                ...
                .function("setTarget", returnsVoid().params(FnLivingEntity.TYPE)) { it.target?.setTarget(it.getRef(0) as LivingEntity) }
                .function("target", returns(FnLivingEntity.TYPE).noParams()) { it.setReturnRef(it.target?.target) }
                .function("hasTarget", returns(Type.Z).noParams()) { it.setReturnBool(it.target?.hasTarget() ?: false) }
                .function("huntingArea", returns(FnBoundingBox.TYPE).noParams()) { it.setReturnRef(it.target?.huntingArea) }
        }
    }
}
