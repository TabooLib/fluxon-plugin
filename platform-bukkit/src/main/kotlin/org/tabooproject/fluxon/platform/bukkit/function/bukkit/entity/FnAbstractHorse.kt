    ...
    private fun init() {
        with(FluxonRuntime.getInstance()) {
            registerExtension(AbstractHorse::class.java)
                ...
                .function("setDomestication", returnsVoid().params(Type.I)) { it.target?.setDomestication(it.getAsInt(0).toInt()) }
                .function("maxDomestication", returns(Type.I).noParams()) { it.setReturnInt(it.target?.maxDomestication ?: 0) }
                .function("setMaxDomestication", returnsVoid().params(Type.I)) { it.target?.setMaxDomestication(it.getAsInt(0).toInt()) }
                .function("jumpStrength", returns(Type.D).noParams()) { it.setReturnDouble(it.target?.jumpStrength ?: 0.0) }
                .function("setJumpStrength", returnsVoid().params(Type.D)) { it.target?.setJumpStrength(it.getAsDouble(0)) }
