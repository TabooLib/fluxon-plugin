    ...
    private fun init() {
        with(FluxonRuntime.getInstance()) {
            registerExtension(Wolf::class.java)
                ...
                .function("variant", returns(org.tabooproject.fluxon.platform.bukkit.function.bukkit.entity.FnWolfVariant.TYPE).noParams()) { it.setReturnRef(it.target?.variant) }
                .function("setVariant", returnsVoid().params(org.tabooproject.fluxon.platform.bukkit.function.bukkit.entity.FnWolfVariant.TYPE)) { it.target?.setVariant(it.getRef(0) as Wolf.Variant) }
        }
    }
}
