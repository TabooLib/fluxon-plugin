    ...
    private fun init() {
        with(FluxonRuntime.getInstance()) {
            registerExtension(org.bukkit.configuration.file.YamlConfigurationOptions::class.java)
                ...
                .function("pathSeparator", returns(org.tabooproject.fluxon.platform.bukkit.function.bukkit.configuration.file.FnYamlConfigurationOptions.TYPE).params(Type.I)) { it.setReturnRef(it.target?.pathSeparator(it.getAsInt(0).toInt().toChar())) }
                ...
                .function("indent", returns(org.tabooproject.fluxon.platform.bukkit.function.bukkit.configuration.file.FnYamlConfigurationOptions.TYPE).params(Type.I)) { it.setReturnRef(it.target?.indent(it.getAsInt(0).toInt())) }
                .function("width", returns(Type.I).noParams()) { it.setReturnInt(it.target?.width() ?: 0) }
                .function("width", returns(org.tabooproject.fluxon.platform.bukkit.function.bukkit.configuration.file.FnYamlConfigurationOptions.TYPE).params(Type.I)) { it.setReturnRef(it.target?.width(it.getAsInt(0).toInt())) }
                // .function("codePointLimit", returns(Type.I).noParams()) { it.setReturnInt(it.target?.codePointLimit() ?: 0) }
                // .function("codePointLimit", returns(org.tabooproject.fluxon.platform.bukkit.function.bukkit.configuration.file.FnYamlConfigurationOptions.TYPE).params(Type.I)) { it.setReturnRef(it.target?.codePointLimit(it.getAsInt(0).toInt())) }
        }
    }
}
