    ...
    private fun init() {
        with(FluxonRuntime.getInstance()) {
            registerExtension(CookingRecipe::class.java)
                .function("setInput",returns(org.tabooproject.fluxon.platform.bukkit.function.bukkit.inventory.FnCookingRecipe.TYPE).params(org.tabooproject.fluxon.platform.bukkit.function.bukkit.FnMaterial.TYPE)) { it.setReturnRef(it.target?.setInput(it.getRef(0) as Material)) }
                ...
                .function("setInputChoice", returns(org.tabooproject.fluxon.platform.bukkit.function.bukkit.inventory.FnCookingRecipe.TYPE).params(org.tabooproject.fluxon.platform.bukkit.function.bukkit.inventory.FnRecipeChoice.TYPE)) { it.setReturnRef(it.target?.setInputChoice(it.getRef(0) as RecipeChoice)) }
                ...
                .function("category", returns(org.tabooproject.fluxon.platform.bukkit.function.bukkit.inventory.recipe.FnCookingBookCategory.TYPE).noParams()) { it.setReturnRef(it.target?.category) }
                .function("setCategory", returnsVoid().params(org.tabooproject.fluxon.platform.bukkit.function.bukkit.inventory.recipe.FnCookingBookCategory.TYPE)) { it.target?.setCategory(it.getRef(0) as CookingBookCategory)  }
                .function("setCategory", returnsVoid().params(Type.STRING)) { org.tabooproject.fluxon.platform.bukkit.function.bukkit.inventory.recipe.FnCookingBookCategory.enumValue(it.getString(0))?.let { p0 -> it.target?.setCategory(p0)  } }
        }
    }
}
