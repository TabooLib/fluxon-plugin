    ...
    private fun init() {
        with(FluxonRuntime.getInstance()) {
            registerExtension(MerchantRecipe::class.java)
                ...
                .function("removeIngredient", returnsVoid().params(Type.I)) { it.target?.removeIngredient(it.getAsInt(0)) }
                ...
                .function("setDemand", returnsVoid().params(Type.I)) { it.target?.setDemand(it.getAsInt(0)) }
                .function("specialPrice", returns(Type.I).noParams()) { it.setReturnInt(it.target?.specialPrice ?: 0) }
                .function("setSpecialPrice", returnsVoid().params(Type.I)) { it.target?.setSpecialPrice(it.getAsInt(0)) }
                .function("uses", returns(Type.I).noParams()) { it.setReturnInt(it.target?.uses ?: 0) }
                .function("setUses", returnsVoid().params(Type.I)) { it.target?.setUses(it.getAsInt(0)) }
                .function("maxUses", returns(Type.I).noParams()) { it.setReturnInt(it.target?.maxUses ?: 0) }
                .function("setMaxUses", returnsVoid().params(Type.I)) { it.target?.setMaxUses(it.getAsInt(0)) }
                ...
                .function("setVillagerExperience", returnsVoid().params(Type.I)) { it.target?.setVillagerExperience(it.getAsInt(0)) }
