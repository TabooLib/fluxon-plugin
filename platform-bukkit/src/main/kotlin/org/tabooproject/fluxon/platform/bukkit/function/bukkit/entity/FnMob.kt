    ...
    private fun init() {
        with(FluxonRuntime.getInstance()) {
            registerExtension(Mob::class.java)
                ...
                .function("setAware", returnsObject().params(Type.OBJECT)) { it.setReturnRef(it.target?.setAware(it.getBool(0))) }
                .function("isAware", returns(Type.Z).noParams()) { it.setReturnRef(it.target?.isAware) }
...
//                .function("isAggressive", returns(Type.Z).noParams()) { it.setReturnRef(it.target?.isAggressive) }
//                .function("setAggressive", returnsObject().params(Type.OBJECT)) { it.setReturnRef(it.target?.isAggressive = it.getBool(0)) }
//                .function("isLeftHanded", returns(Type.Z).noParams()) { it.setReturnRef(it.target?.isLeftHanded) }
