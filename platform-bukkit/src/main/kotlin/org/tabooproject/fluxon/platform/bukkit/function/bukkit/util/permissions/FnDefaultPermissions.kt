    ...
    private fun init() {
        with(FluxonRuntime.getInstance()) {
            registerExtension(DefaultPermissions::class.java)
                // 1 参数: Permission
                .function("registerPermission", returnsObject().params(Type.OBJECT)) {
                    DefaultPermissions.registerPermission(it.getRef(0) as Permission)
                }
                // 2 参数: Permission + Boolean
                .function("registerPermission", returnsObject().params(Type.OBJECT, Type.Z)) {
                    DefaultPermissions.registerPermission(it.getRef(0) as Permission, it.getBool(1))
                }
                ...
                // 2 参数: String + String
                .function("registerPermission", returnsObject().params(Type.STRING, Type.STRING)) {
                    DefaultPermissions.registerPermission(it.getString(0)!!, it.getString(1))
                }
