    ...
    private fun init() {
        with(FluxonRuntime.getInstance()) {
            registerExtension(Scoreboard::class.java)
                .syncFunction("registerNewObjective", returnsObject().params(Type.STRING, Type.OBJECT)) {
                    it.setReturnRef(it.target?.registerNewObjective(it.getString(0)!!, it.getString(1)!!))
                }
                .syncFunction("registerNewObjective", returnsObject().params(Type.STRING, Type.OBJECT, Type.STRING)) {
                ...
                .syncFunction("registerNewObjective", returnsObject().params(Type.STRING, Type.OBJECT, Type.STRING, Type.OBJECT)) {
                    it.setReturnRef(when (val var2 = it.getRef(1)) {
                ...
                .function("objectives", returnsObject().noParams()) { it.setReturnRef(it.target?.objectives) }
                ...
                .function("getPlayerTeam", returnsObject().params(Type.OBJECT)) { it.setReturnRef(it.target?.getPlayerTeam(it.getRef(0) as OfflinePlayer)) }
                .function("getEntryTeam", returnsObject().params(Type.OBJECT)) { it.setReturnRef(it.target?.getEntryTeam(it.getString(0)!!)) }
                .function("getTeam", returnsObject().params(Type.OBJECT)) { it.setReturnRef(it.target?.getTeam(it.getString(0)!!)) }
                .function("teams", returnsObject().noParams()) { it.setReturnRef(it.target?.teams) }
                .function("registerNewTeam", returnsObject().params(Type.OBJECT)) { it.setReturnRef(it.target?.registerNewTeam(it.getString(0)!!)) }
                ...
                .function("clearSlot", returnsObject().params(Type.OBJECT)) { it.setReturnRef(it.target?.clearSlot(it.getRef(0) as DisplaySlot)) }
        }
    }
}
