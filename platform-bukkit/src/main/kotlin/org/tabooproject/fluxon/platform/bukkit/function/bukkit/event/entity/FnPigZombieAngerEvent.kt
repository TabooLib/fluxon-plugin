    ...
    private fun init() {
        with(FluxonRuntime.getInstance()) {
            registerExtension(PigZombieAngerEvent::class.java)
                ...
                .function("newAnger", returnsObject().noParams()) { it.setReturnRef(it.target?.newAnger) }
                .function("setNewAnger", returnsObject().params(Type.OBJECT)) { it.setReturnRef(it.target?.setNewAnger(it.getInt(0).toInt())) }
