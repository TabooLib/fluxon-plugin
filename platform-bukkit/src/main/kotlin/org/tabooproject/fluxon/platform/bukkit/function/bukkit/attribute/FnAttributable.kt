    ...
    private fun init() {
        with(FluxonRuntime.getInstance()) {
            registerExtension(Attributable::class.java)
                ...
                .syncFunction("setBaseArmor",returns(FnAttributeInstance.TYPE).params(Type.D)) { it.setReturnRef(it.target?.getAttribute(Attribute.GENERIC_ARMOR)?.apply { baseValue = it.getDouble(0) }) }
                ...
                .syncFunction("setBaseLuck",returns(FnAttributeInstance.TYPE).params(Type.D)) { it.setReturnRef(it.target?.getAttribute(Attribute.GENERIC_LUCK)?.apply { baseValue = it.getDouble(0) }) }
