    ...
    private fun init() {
        with(FluxonRuntime.getInstance()) {
            registerExtension(Fox::class.java)
                ...
                .function("setFoxType", returnsVoid().params(Type.OBJECT)) { it.target?.setFoxType(it.getRef(0) as Fox.Type) }
                ...
                .function("setFirstTrustedPlayer", returnsVoid().params(Type.OBJECT)) { it.target?.setFirstTrustedPlayer(it.getRef(0) as AnimalTamer) }
                .function("secondTrustedPlayer", returnsObject().noParams()) { it.setReturnRef(it.target?.secondTrustedPlayer) }
                .function("setSecondTrustedPlayer", returnsVoid().params(Type.OBJECT)) { it.target?.setSecondTrustedPlayer(it.getRef(0) as AnimalTamer) }
