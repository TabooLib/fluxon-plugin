    ...
    private fun init() {
        with(FluxonRuntime.getInstance()) {
            registerExtension(FurnaceRecipe::class.java)
                .function("setInput", returnsVoid().params(Type.OBJECT)) {
                ...
                .function("setInput", returnsVoid().params(Type.OBJECT, Type.I)) {
                    it.target?.setInput(it.getRef(0) as Material, it.getInt(1).toInt())
                }
                .function("setInputChoice", returnsVoid().params(Type.OBJECT)) { it.target?.setInputChoice(it.getRef(0) as RecipeChoice) }
        }
    }
}
